...
def create_sf_num(sequence: str) -> Node:
...
combos = permutations(raw_inputs, 2)
highest_mag = 0
for combo in combos:
    left_raw, right_raw = combo

    left_sf = create_sf_num(left_raw)
    right_sf = create_sf_num(right_raw)

    sf_num = sf_add(left_sf, right_sf)
    magnitude = get_sf_magnitude(sf_num)
